  ...
  def __init__(self, app: FastAPI):
    ...
    def getMyModelList(request: Request):
      ...
      with session_scope(True) as session:
        ...
        # 查询模型：用户创建的 + 系统内置的(userId为None)
        models = session.query(ModelPrvdModl).where(
          ModelPrvdModl.prvdId.in_(prvdIds), 
          or_(ModelPrvdModl.userId == userId, ModelPrvdModl.userId.is_(None))
        ).all()
        items = [{
    ...
    # 同步 Ollama 已下载的模型
    @app.post('/sys/model/ollama/sync')
    def syncOllamaModels(request: Request):
      userId = self.getUserId(request)
      prvdId = 'ollama'
      ...
      with session_scope(True) as session:
        params = session.query(ModelParam).where(
          ModelParam.prvdId == prvdId, 
          ModelParam.userId == userId,
          ModelParam.modlId.is_(None)
        ).all()
        for param in params:
          if param.prmCd == 'base_url' and param.prmVal:
            ollamaBaseUrl = param.prmVal
            break
      
      # 从 Ollama API 获取已下载的模型列表
      try:
        response = requests.get(f'{ollamaBaseUrl}/api/tags', timeout=5)
        ollamaModels = response.json().get('models', [])
      except Exception as e:
        raise BaseBusiException(f'无法连接 Ollama 服务: {str(e)}')
      
      syncedCount = 0
      with session_scope() as session:
        # 确保用户有 ollama 的 setting
        modelSetting = ModelSetting(userId=userId, prvdId=prvdId)
        session.merge(modelSetting)
        
        # 获取用户已有的模型
        existingModels = session.query(ModelPrvdModl).where(
          ModelPrvdModl.prvdId == prvdId,
          or_(ModelPrvdModl.userId == userId, ModelPrvdModl.userId.is_(None))
        ).all()
        existingModelNames = {m.modlNm for m in existingModels}
        
        # 添加新模型
        for ollamaModel in ollamaModels:
          modelName = ollamaModel.get('name', '')
          if modelName and modelName not in existingModelNames:
            modlId = self.getPk()
            newModel = ModelPrvdModl(
              modlId=modlId,
              prvdId=prvdId,
              modlNm=modelName,
              modlTyp='llm',
      ...
      return self.success({'synced': syncedCount, 'total': len(ollamaModels)})