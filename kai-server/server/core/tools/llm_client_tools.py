...
class LLMClient:
  ...
  def get_llm_client(self):
    # 仅支持 Ollama
    return OllamaLLM(model=self.model, base_url=self.args.get('base_url', None), temperature=self.temperature)
  ...
  def get_embedding_function(self):
    ...
    if self.provider == 'ollama':
      ...
      if (not model_dir.endswith('/')):
        model_dir = model_dir + '/'
      model_name = model
      if (not model_name.startswith(model_dir)):
