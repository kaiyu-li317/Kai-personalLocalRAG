<div align="center"><div align="center"># WenKB - 智能个人知识库桌面应用
...
**Next-Generation Knowledge Management Tool Powered by LLM**# 📚 KAI - 智能个人知识库WenKB 是一款基于大语言模型（LLM）的下一代知识管理工具，融合 AI 能力重新定义知识组织方式。支持多源知识整合、智能问答、自动化知识加工和可视化知识网络，助力构建您的第二大脑。
...
- **Multi-format Import** - Support PDF, Word, Markdown, web links, plain text and more- **双模式编辑器**：Markdown + 富文本混合编辑

- **AI Auto-processing** - Automatic segmentation, summary generation, Q&A pair creation, knowledge extraction

- **Document Management** - Version control, knowledge linking, batch operations[功能特性](#-功能特性) • [快速开始](#-快速开始) • [项目结构](#-项目结构) • [配置说明](#-配置说明) • [开发指南](#-开发指南)
...
- **Local Deployment** - Fully local data storage, privacy protection- **对话式交互**：支持追问、溯源引用、多轮知识推理
...
- **Chinese Optimized** - Built-in M3E Chinese embedding model for better semantic understanding---
...
| Dependency | Version | Description |- **多格式导入** - 支持 PDF、Word、Markdown、网页链接、纯文本等多源数据```
...
cd Kai-personalLocalRAG### 🔍 智能搜索与问答│   │   ├── components/    # 通用组件
...
curl -fsSL https://ollama.com/install.sh | sh### 🤖 AI 能力├── wenkb-server/          # 后端 (Python FastAPI)



# Start service and download model- **本地 LLM** - 集成 Ollama，支持 Qwen、Llama、Mistral 等模型│   ├── server/

ollama serve

ollama pull qwen2- **中文优化** - 内置 M3E 中文嵌入模型，语义理解更精准│   │   ├── api/           # API 接口
...
After successful startup, visit: **http://localhost:11420**
...
├── kai-client/                # Frontend project

│   ├── src/cd kai

│   │   ├── views/            # Page components

│   │   ├── components/       # Common components```- **Node.js** >= 18.x
...
├── kai-server/                # Backend project```bash### 一键启动
...
│   │   ├── db/               # Database operationsbrew install ollama```bash
...
│   ├── resources/            # Resource directory# Linux

│   │   ├── database/         # SQLite database

│   │   ├── documents/        # Uploaded documentscurl -fsSL https://ollama.com/install.sh | sh# 启动所有服务 (Ollama + 后端 + 前端)
...
├── start.sh                   # One-click start scriptollama serve# 停止所有服务

├── stop.sh                    # Stop script

└── README.mdollama pull qwen2./stop.sh
